...
void turn_On_PPI_to_GPIO_for_PWM(uint32_t ulPin, uint32_t gpiote_channel, NRF_TIMER_Type* Timer, uint32_t CC_channel)
{
  ...
  int chan_1 = find_free_PPI_channel(chan_0);
    ...
    simblee_ppi_channel_assign(chan_0, &((*Timer).EVENTS_COMPARE[CC_channel]), &NRF_GPIOTE->TASKS_OUT[gpiote_channel]);
    ...
    simblee_ppi_channel_assign(chan_1, &((*Timer).EVENTS_COMPARE[3]), &NRF_GPIOTE->TASKS_OUT[gpiote_channel]);
		// Save PPI channel number
...
void turn_Off_GPIOTE_PPI_from_GPIO(uint32_t ulPin)
{
  ...
  {
    simblee_ppi_channel_unassign(PPI_Channels_Occupied[ulPin][0]);
    PPI_Channels_Occupied[ulPin][0] = 255;
  }

  if (PPI_Channels_Occupied[ulPin][1] < 255)
  {
    simblee_ppi_channel_unassign(PPI_Channels_Occupied[ulPin][1]);
  ...
  if (ulValue == 255)
  {
	...
	else
	{
      ...
      digitalWrite(ulPin, LOW);
            ...
            dynamic_attachInterrupt(TIMER1_IRQn, TIMER1_Interrupt);
