...
- (NSString*)incrementUUID:(CBUUID*)uuid amount:(uint8_t)amount
{
...
- (void)connect:(NSArray *)services
{
    ...
    if (_simbleeManager.central.state != CBCentralManagerStatePoweredOn) {
        DLog(@"delaying connect until ble powered on");
        Simblee *this = self;
...
- (void)signal:(uint8_t)number data:(NSData *)data
{
    ...
    if (_state == SimbleeStateConnected) {
        DLog(@"writing to signal characteristic");
